...
class Features(torch.nn.Module):
    ...
    def save_prediction_maps(self, output_path, rgb_path, save_num=5):
        for i in range(max(save_num, len(self.predictions))):            
            ...
            # print("第 i 个 RGB 图像的路径",rgb_path[i][0])
            # print("第 i 个 RGB 图像的路径下的元素", rgb_path[i][0].split('/'))
            class_dir = os.path.join(output_path, rgb_path[i][0].replace('\\', '/').split('/')[-5])
            print("第 i 个 RGB 图像的路径", class_dir)
            if not os.path.exists(class_dir):
                os.makedirs(class_dir)
            ad_dir = os.path.join(class_dir, rgb_path[i][0].replace('\\', '/').split('/')[-3])
            if not os.path.exists(ad_dir):
                os.makedirs(ad_dir)
            
            plt.savefig(os.path.join(ad_dir,  str(self.image_preds[i]) + '_pred_' + rgb_path[i][0].replace('\\', '/').split('/')[-1] + '.jpg'))
            plt.close('all')
