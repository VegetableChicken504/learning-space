...
"""
action参数：当 sh 命令行（终端）中出现具有 action 属性的参数时，该参数的值就是‘store_true’所表示的 ture；当 sh 命令行中没有出现具有 action 属性的参数时，
该参数的值为‘store_true’所表示的相对值 false
default参数：字面意思，值为true则默认为true，反之亦然。

当两个参数同时出现时，default和action的优先级比较：
1、当action属性的参数未在命令行出现即未触发状态时）：default优先级 > action优先级
2、命令行有action属性的参数时（触发状态：--参数）：action优先级 > default优先级
"""
